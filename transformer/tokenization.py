...
class BertTokenizer(object):
    ...
    def __init__(
        ...
        self.vocab: OrderedDict = load_vocab(vocab_file)
        self.ids_to_tokens: OrderedDict = OrderedDict([(ids, tok) for tok, ids in self.vocab.items()])
        self.do_basic_tokenize: bool = do_basic_tokenize
        if do_basic_tokenize:
            self.basic_tokenizer = BasicTokenizer(do_lower_case=do_lower_case, never_split=never_split)
        self.wordpiece_tokenizer = WordpieceTokenizer(vocab=self.vocab)
        self.max_len: int = max_len if max_len is not None else int(1e12)
        self.basic_only: bool = basic_only
